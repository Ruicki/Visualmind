...
const MARQUEE_STYLE_ID = 'hero-ticker-marquee-style';
...
export default function TemplateTicker({ campaign, timeLeft, isActive }) {
    const accentColor = campaign.accent_color || '#ffffff';
    const btnTextColor = getContrastColor(accentColor);
    const tickerTextColor = getContrastColor(accentColor);
    const ctaLink = campaign.button_link || `/shop?campaign=${campaign.slug}`;

    // Repeat name 6 times for seamless loop (3 visible + 3 hidden for the -50% trick)
