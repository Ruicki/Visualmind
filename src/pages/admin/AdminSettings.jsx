...
import { User, Shield, Check, Loader, AlertTriangle } from 'lucide-react';
...
export default function AdminSettings() {
    ...
    const handlePromote = async (e) => {
        ...
        try {
            ...
            if (searchError || !profiles) throw new Error(t('admin.user_not_found'));
            ...
            setMessage({ type: 'success', text: t('admin.success_promote') });
            setEmailToPromote('');
        } catch (error) {
    ...
    return (
        <div style={{ maxWidth: '800px' }}>
            <h2 style={{ fontSize: '1.8rem', fontWeight: '800', marginBottom: '2rem' }}>{t('admin.settings_title')}</h2>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '2rem', borderRadius: '24px', border: '1px solid var(--border-light)', marginBottom: '3rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                    ...
                    <div>
                        <h3 style={{ fontSize: '1.2rem', marginBottom: '0.2rem' }}>{t('admin.admin_mgmt')}</h3>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{t('admin.admin_desc')}</p>
                    </div>
                </div>

                <div style={{ background: 'rgba(255,255,255,0.03)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-light)' }}>
                    <h4 style={{ fontSize: '1rem', marginBottom: '1rem' }}>{t('admin.promote_title')}</h4>
                    <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
                        {t('admin.promote_desc')}
                        <strong> {t('admin.promote_warn')}</strong>
                    </p>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '2rem', borderRadius: '24px', border: '1px solid var(--border-light)' }}>
                <h3 style={{ fontSize: '1.2rem', marginBottom: '1rem' }}>{t('admin.your_profile')}</h3>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'rgba(255,255,255,0.03)', borderRadius: '16px' }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                            {t('admin.role_label')}: <span style={{ color: '#3b82f6', fontWeight: 'bold', textTransform: 'capitalize' }}>{currentUserRole}</span>
                        </div>
                    </div>
                </div>
            </div>
